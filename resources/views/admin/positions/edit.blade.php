                                ...
                                <div class="tab-content" id="myTabContent">
                                    @foreach($langs as $lang)
                                        <div class="tab-pane mt-3 fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}" role="tabpanel" aria-labelledby="{{ $lang->code }}-tab">
                                            <div class="form-group">
                                                ...
                                                <input type="text" {{ $lang->code == $main_lang->code ? 'required' : '' }} class="form-control @error('title.'.$lang->code) is-invalid @enderror" name="name[{{ $lang->code }}]" value="{{ old('name.'.$lang->code) ?? $position->name[ $lang->code ] ?? null}}" id="title" placeholder="Nomi...">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="active" class="form-select">
                                            <option value="1" {{ $position->active == 1 ? 'selected' : '' }}>Faol</option>
                                            <option value="0" {{ $position->active == 0 ? 'selected' : '' }}>Nofaol</option>
                                        </select>

                                    </div>
                                    <div class="form-group">
                                        <label for="subtitle" class="form-label">Tartib</label>
                                        <input type="text" class="form-control " name="order" value="{{ old('order') ?? $position->order }}" id="subtitle" placeholder="Tartib...">
                                        @error('order')
                                        <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
