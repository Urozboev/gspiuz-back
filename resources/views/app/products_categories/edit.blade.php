                    ...
                    <form method="post" action="{{ route($route_name . '.update', [$route_parameter => $productsCategory]) }}" enctype="multipart/form-data" id="add">
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <div class="tab-content" id="myTabContent">
                                    ...
                                    <div class="tab-pane mt-3 fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}" role="tabpanel" aria-labelledby="{{ $lang->code }}-tab">
                                        <div class="form-group">
                                            ...
                                            <input type="text" {{ $lang->code == $main_lang->code ? 'required' : '' }} class="form-control @error('title.'.$lang->code) is-invalid @enderror" name="title[{{ $lang->code }}]" value="{{ old('title.'.$lang->code) ?? $productsCategory->title[$lang->code] ?? null }}" id="title" placeholder="Nomi...">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="desc[{{ $lang->code }}]" id="desc" cols="30" rows="10" class="form-control @error('desc.'.$lang->code) is-invalid @enderror ckeditor" name="desc[{{ $lang->code }}]" placeholder="Tavsif...">{{ old('desc.'.$lang->code) ?? $productsCategory->desc[$lang->code] ?? null }}</textarea>
                                ...
                                <div class="form-group">
                                    <label for="in_main" class="form-label">Birinchi ekranda koʻrsatish</label>
                                    <select class="form-select @error('in_main') is-invalid @enderror" id="in_main" name="in_main">
                                        <option value="0" {{ $productsCategory->in_main == 0 ? 'selected' : '' }}>Yoʻq</option>
                                        <option value="1" {{ $productsCategory->in_main == 1 ? 'selected' : '' }}>Ha</option>
                                    </select>
                                    @error('in_main')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
